...
export default function GallerySection() {
  return (
    <section id="galeria" className="gallery-section bg-ink py-16 lg:min-h-screen lg:flex lg:items-center">
      <div className="max-w-275 mx-auto w-full px-6">
        {/* Heading — acima do layout em ambos os breakpoints */}
        ...
        {/* Mobile: flex-col-reverse → phone visualmente acima do feed | Desktop: grid 2 colunas */}
        <div className="flex flex-col-reverse lg:grid lg:grid-cols-[1fr_380px] lg:gap-16 lg:items-center">
          {/* Feed — último visualmente no mobile (flex-col-reverse), esquerda no desktop */}
          <ul className="mt-6 lg:mt-0 flex flex-col gap-2.5 list-none m-0 p-0">
            {PHOTOS.map((p) => (
              <li
                key={p.id}
              ...
              >
                {/* Thumbnail */}
                <div className="w-13.5 h-13.5 rounded-xl overflow-hidden shrink-0 bg-white/8 relative">
                  <Image
          ...
          {/* Phone — primeiro visualmente no mobile (flex-col-reverse), direita/sticky no desktop */}
          <div className="gallery-phone-sticky flex justify-center mb-8 lg:mb-0 lg:sticky lg:top-30 will-change-transform">
            <PhoneMockup width={260}>
              <div className="absolute inset-0 flex flex-col" style={{ background: "#0d0d0d" }}>
                ...
                {/* Grade 2 colunas — phone-thumb animados pelo GSAP no desktop */}
                  ...
                  {PHOTOS.map((p, i) => (
                    ...
                    >
                      <Image
                        ...
                        loading="lazy"
                      />
                      <div className="absolute bottom-0 left-0 right-0 pt-3.5 px-1.5 pb-1 bg-linear-to-t from-black/65 to-transparent text-[10px] text-white font-sans font-medium tracking-[0.01em]">
                        {p.name}
                      </div>
                    </div>
                  ))}
